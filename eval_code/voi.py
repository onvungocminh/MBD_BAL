...
def voi(reconstruction_path, groundtruth_path, ignore_reconstruction=[], ignore_groundtruth=[0]):
    ...
    """
    reconstruction = np.array(Image.open(reconstruction_path))
    groundtruth = np.array(Image.open(groundtruth_path))

    # cv2.imwrite('abc.png', reconstruction)
    # pdb.set_trace()
